...
export default function Events() {
  ...
  return (
    ...
    >
      {/* Header */}
      <div className="mb-8">
        <h1 className="text-3xl font-bold mb-2">Career Events</h1>
        <p className={`text-lg ${isDark ? 'text-gray-400' : 'text-gray-600'}`}>
          Discover networking events, job fairs, and workshops
        </p>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {filteredEvents.length === 0 ? (
          <div className="text-center py-12 px-4 col-span-1">
          ...
          filteredEvents.map((event) => (
            <Card key={event.id} className="p-4 flex flex-col justify-between h-full">
      ...
      {filteredEvents.length > 0 && (
        <div className={`border-t pt-6 mt-6 ${isDark ? 'border-gray-800' : 'border-gray-200'}`}>
          <div className="flex items-center space-x-2 mb-4">
            <TrendingUp className="h-5 w-5 text-blue-500" />
            <h3 className="font-bold text-lg">Trending Events</h3>
          </div>
